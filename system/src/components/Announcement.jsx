...
const Announcement = () => {
  ...
  // ---------- Simulation state ----------
  const [waterLevel, setWaterLevel] = useState(6.5);
  ...
  // Listen for custom event when thresholds change (from SystemTab)
  useEffect(() => {
    const handleThresholdsChange = (event) => {
      setThresholds(event.detail);
    };
    window.addEventListener('thresholdsChanged', handleThresholdsChange);
